...
1. **Clone the repository**:
   ```bash
   git clone -b LAB2 https://github.com/bpbb/FRA502-LAB-6631.git
    ```
...
2. **Run all nodes**:
   ...
   rviz2
   ```
   Don't forget to source in each terminal.
   In RViz2, open the config file (lab2.rviz) located in FRA502-LAB-6631/src/lab2.rviz
   
3. **Spawn Turtle2**
   ```bash
   ros2 service call /spawn_turtle turtlesim/srv/Spawn "{x: 5.0, y: 5.0, theta: 0.0, name: 'turtle2'}" 
   ```
   The spawned turtle will start moving when the pizza count reaches its maximum value (default = 5).
   To change the maximum value, modify the variable self.max_pizza inside eater.py and killer.py.
